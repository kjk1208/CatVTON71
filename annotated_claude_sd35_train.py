...
def from_uint8(img: Image.Image, size_hw: Tuple[int, int]) -> torch.Tensor:
    """
    PIL 이미지를 PyTorch 텐서로 변환 ([-1, 1] 범위로 정규화)
    
    Args:
        img (Image.Image): 입력 PIL 이미지
        size_hw (Tuple[int, int]): 목표 크기 (height, width)
    
    Returns:
        torch.Tensor: 정규화된 이미지 텐서 [C, H, W] 범위 [-1, 1]
    
    과정:
    1. RGB 변환 및 크기 조정 (BICUBIC 보간)
    2. numpy 배열 → PyTorch 텐서
    3. [H, W, C] → [C, H, W] 차원 변경
    4. [0, 255] → [0, 1] → [-1, 1] 정규화
    """
...
def load_mask(p: str, size_hw: Tuple[int, int]) -> torch.Tensor:
    ...
    Returns:
        torch.Tensor: 이진 마스크 텐서 [1, H, W] 범위 [0, 1]
    
    과정:
    1. 그레이스케일로 변환 및 크기 조정 (NEAREST 보간)
    2. 0.5 임계값으로 이진화 (0 또는 1)
    3. 채널 차원 추가 [H, W] → [1, H, W]
...
class DotDict(dict):
    """
    딕셔너리를 점(.) 표기법으로 접근할 수 있게 하는 유틸리티 클래스
    
    사용 예:
        cfg = DotDict({"lr": 1e-4, "batch_size": 16})
        print(cfg.lr)         # 1e-4 (dict['lr']과 동일)
        cfg.new_param = 42    # dict['new_param'] = 42와 동일
    
    목적:
    - 설정(config) 딕셔너리를 객체처럼 사용 가능
    - cfg['key'] 대신 cfg.key로 간편하게 접근
    """
...
def maybe_init_distributed() -> Dict[str, int]:
    """
    환경 변수를 확인하여 분산 학습을 초기화
    
    Returns:
        Dict[str, int]: 분산 학습 정보
        - is_dist: 분산 학습 여부
        - rank: 현재 프로세스 순위 (0이 main)
        - world_size: 전체 프로세스 수
        - local_rank: 로컬 GPU 번호
    
    동작:
    1. RANK 환경변수 확인
    2. 있으면 NCCL 백엔드로 분산 학습 초기화
    3. 없으면 단일 GPU 모드
    
    사용:
    - torchrun으로 실행 시 자동으로 분산 학습 활성화
    - 일반 python 실행 시 단일 GPU 모드
    """
...
def get_distributed_info() -> Dict[str, int]:
    ...
    Returns:
        Dict[str, int]: 현재 프로세스의 분산 학습 정보
    
    용도:
    - 학습 중 현재 프로세스의 역할 확인
    - 로깅이나 체크포인트 저장을 main process(rank=0)에서만 수행
    """
...
class PairListDataset(Dataset):
    ...
    - person: 사람 이미지 경로
    - garment: 의상 이미지 경로  
    - mask: 마스크 이미지 경로 (선택사항)
    
    출력:
    - x_concat_in: 입력 이미지 [사람(마스킹) + 의상] 연결
    - x_concat_gt: 타겟 이미지 [사람(완성) + 의상] 연결
    - m_concat: 마스크 [사람 마스크 + 의상 마스크] 연결
    """
...
# ================================================================
# SD3.5 VAE 인코딩/디코딩 헬퍼 함수들
# ================================================================

@torch.no_grad()
def to_latent_sd3(vae, x, sample=True, generator=None):
    """
    SD3.5 VAE를 사용하여 이미지를 latent space로 인코딩
    
    Args:
        vae: SD3.5 VAE 모델
        x (torch.Tensor): 입력 이미지 텐서 [-1, 1] 범위
        sample (bool): True면 확률적 샘플링, False면 평균값 사용
        generator: 랜덤 시드 제어용 generator
    
    Returns:
        torch.Tensor: 인코딩된 latent 텐서 [B, 16, H/8, W/8]
    
    과정:
    1. VAE 인코더로 posterior distribution 계산
    2. 샘플링 또는 평균값 추출
    3. SD3.5 전용 scaling/shift factor 적용
    
    중요: SD3.5는 특별한 scaling_factor(1.5305)와 shift_factor(0.0609) 사용
    """
...
def from_latent_sd3(vae, z: torch.Tensor) -> torch.Tensor:
    """
    SD3.5 VAE를 사용하여 latent를 이미지로 디코딩
    
    Args:
        vae: SD3.5 VAE 모델
        z (torch.Tensor): latent 텐서 [B, 16, H/8, W/8]
    
    Returns:
        torch.Tensor: 디코딩된 이미지 텐서 [-1, 1] 범위
    
    과정:
    1. scaling/shift factor 역적용 (인코딩의 역과정)
    2. VAE 디코더로 이미지 생성
    
    중요: to_latent_sd3()의 정확한 역변환
    """
    vdtype = next(vae.parameters()).dtype
    z = z.to(vdtype)
...
# ================================================================
# FlowMatch 스케줄러 설정 함수들
# ================================================================

def create_enhanced_fm_scheduler(base_scheduler_config, device, height, width, transformer):
    """
    SD3.5에 최적화된 FlowMatch 스케줄러 생성
    
    Args:
        base_scheduler_config: 기본 스케줄러 설정
        device: 실행 디바이스
        height, width: 이미지 크기
        transformer: SD3.5 Transformer 모델
    
    Returns:
        tuple: (스케줄러, mu 값)
    
    기능:
    1. SD3.5에 최적화된 설정으로 FlowMatch 스케줄러 생성
    2. Dynamic shifting을 위한 mu 값 계산
    3. 이미지 해상도에 따른 적응적 노이즈 스케줄링
    
    핵심 설정:
    - num_train_timesteps: 1000 (표준 diffusion 스텝)
    - shift: 3.0 (더 부드러운 노이즈 스케줄)
    - dynamic shifting: 해상도에 따른 적응적 조정
    """
...
def setup_enhanced_timesteps(scheduler, num_steps, device, mu=None):
    """
    FlowMatch 스케줄러의 timesteps를 안정적으로 설정
    
    Args:
        scheduler: FlowMatch 스케줄러
        num_steps (int): 추론 스텝 수
        device: 실행 디바이스
        mu (float, optional): dynamic shifting 파라미터
    
    Returns:
        tuple: (설정된 스케줄러, timesteps 텐서)
    
    기능:
    1. Dynamic shifting 지원 (mu 파라미터 사용)
    2. Timesteps 정규화 및 검증
    3. 내림차순 정렬 보장 (1000 → 0)
    4. 마지막 timestep이 0이 되도록 보장
    
    중요: FlowMatch는 큰 timestep에서 작은 timestep으로 진행해야 함
    """
    try:
        if mu is not None:
...
class EnhancedCatVTON_SD3_Trainer:
    """
    SD3.5 기반 CatVTON 모델 학습을 위한 메인 트레이너 클래스
    ...
    학습 프로세스:
    1. 배치 로드 → VAE 인코딩 → latent space 변환
    2. 랜덤 timestep 샘플링 → 노이즈 주입
    3. 마스크 기반 inpainting 입력 구성
    4. Transformer로 denoising 예측
    5. Loss 계산 (HOLE 영역에서만)
    6. Gradient 업데이트
    7. 주기적 프리뷰 생성
    
    핵심 특징:
    - FlowMatch 기반 안정적인 학습
    - Inpainting 방식의 의상 합성
    - Self-attention만 학습 (효율적 fine-tuning)
    - 실시간 학습 진행 모니터링
    """
    ...
    def _enhanced_preview_sample(self, num_steps: int, start_latents: torch.Tensor, 
        ...
        """
        프리뷰 이미지 생성을 위한 추론 함수
        
        Args:
            num_steps (int): 추론 스텝 수 (일반적으로 28)
            start_latents (torch.Tensor): 시작 latent 
                - pred(input): Xi_lat (마스킹된 입력)
                - pred(GT): z0_lat (완성된 GT)
            pixels_for_keep (torch.Tensor): 최종 합성 시 사용할 픽셀
            m (torch.Tensor): 마스크 텐서
            shared_noise: 공유 노이즈 (일관된 비교를 위해)
        
        Returns:
            torch.Tensor: 생성된 프리뷰 이미지 [0, 1] 범위
        
        추론 과정:
        1. FlowMatch 스케줄러로 timesteps 설정
        2. preview_strength에 따른 시작점 결정
        3. 반복적 denoising (큰 timestep → 작은 timestep)
        4. 각 스텝마다 inpainting 재합성
        5. 최종 VAE 디코딩 및 픽셀 합성
        
        중요: 학습 시와 동일한 prediction type 처리 사용
        """
        ...
        # 인페인팅 초기화: 학습과 동일한 방식 사용
        # KEEP 영역은 입력 기반, HOLE 영역은 타겟 기반 (학습과 일치)
        z0_noisy_t0 = self._consistent_add_noise(start_latents.float(), shared_noise, t0)
        z = Mi * start_noisy + (1.0 - Mi) * z0_noisy_t0
        ...
        try:
            for i in range(len(timesteps_sub) - 1):
                ...
                # 인페인팅 재합성: 학습과 동일한 방식
                if t_next > 1e-7:
                    # KEEP 영역: 입력 기반, HOLE 영역: 타겟 기반 (학습과 일치)
                    Xi_noisy_next = self._consistent_add_noise(start_latents.float(), shared_noise, t_next)
                    z0_noisy_next = self._consistent_add_noise(start_latents.float(), shared_noise, t_next)  # 같은 타겟 사용
                    z = Mi * Xi_noisy_next + (1.0 - Mi) * z_next
                else:
    ...
    def enhanced_step(self, batch, global_step: int):
        """
        단일 학습 스텝 실행 - VTON 학습의 핵심 로직
        
        Args:
            batch: 데이터 배치 (x_concat_in, x_concat_gt, m_concat)
            global_step (int): 현재 전역 스텝 번호
        
        Returns:
            tuple: (loss, debug_info) 또는 None (실패 시)
        
        학습 과정:
        1. 이미지들을 VAE로 latent space 변환
        2. 랜덤 timestep 샘플링 (전체 범위에서)
        3. 노이즈 주입하여 noisy latents 생성
        4. 마스크 기반 inpainting 입력 구성:
           - KEEP 영역: 입력 이미지 기반 노이즈
           - HOLE 영역: 타겟 이미지 기반 노이즈
        5. Transformer로 epsilon 또는 x0 예측
        6. 예측값을 x0로 변환
        7. HOLE 영역에서만 L2 loss 계산
        8. 가중치 적용 및 정규화
        
        핵심 아이디어:
        - 모델이 HOLE 영역에 의상을 자연스럽게 합성하도록 학습
        - KEEP 영역은 원본 유지, HOLE 영역만 변경
        - FlowMatch 스케줄링으로 안정적인 학습
        """
        H, W = self.cfg.size_h, self.cfg.size_w
        ...
        # 인페인팅 입력 구성 (KEEP 영역은 입력, HOLE 영역은 타겟 기반)
    ...
    def train(self):
        ...
        while global_step < self.cfg.max_steps:
            ...
            dbg_acc = {
                ...
                "target_min": 0.0, "target_max": 0.0, "pred_type": ""
            }
